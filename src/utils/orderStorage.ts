...
export const orderContextStorage = {
	get(): CustomerOrderContext {
		return safeParse<CustomerOrderContext>(
			localStorage.getItem(ORDER_CONTEXT_KEY),
			{},
		);
	},
	...
	/**
	 * Đủ ngữ cảnh giao/nhận để xem sản phẩm / dùng giỏ (Home, Cart).
	 *
	 * DELIVERY: chỉ cần map + text; addressId có thể được resolve khi vào Checkout.
	 */
	isContextSufficientForShopping(ctx?: CustomerOrderContext) {
	...
	/**
	 * Khớp contract BE khi tạo đơn: DELIVERY bắt buộc addressId; dùng trước thanh toán (Checkout).
	 */

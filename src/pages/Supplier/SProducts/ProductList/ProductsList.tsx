...
const ProductsList = () => {
  ...
  const supermarketId =
    session?.user?.marketStaffInfo?.supermarket?.supermarketId ?? "123"
  ...
  const [pricingSuggestion, setPricingSuggestion] =
    useState<PricingSuggestionResponse | null>(null)
  ...
  const handleConfirmPriceClick = async (lot: ProductLotUI) => {
    ...
    try {
      const res = await productService.pricingSuggestion(
        lot.productId,
        lot.originalPrice
      )
  ...
  const handleConfirmPrice = async (
    ...
    try {
      ...
      await productService.confirmPrice(selectedLot.productId, {
        finalPrice,
        acceptedSuggestion: acceptsSuggestion,
        confirmedBy: userId,
      })
  ...
  const handlePublishClick = async (lot: ProductLotUI) => {
    try {
      const userId = session?.user?.userId ?? ""
      await productService.publish(lot.productId, userId)
